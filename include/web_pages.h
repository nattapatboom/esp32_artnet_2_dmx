...
<div class="w">
  ...
  <div id="pane-net" class="pane on">
    ...
    <form id="cfgForm" onsubmit="saveSettings(event)">
      ...
      <div class="f">
        <label class="cklbl"><input type="checkbox" id="i2c_enabled" checked onchange="toggleI2c()"> Enable I2C Bus</label>
      </div>
      <div id="i2c-config" class="fg">
        <div class="f">
      ...
      <div class="f">
        <label class="cklbl"><input type="checkbox" id="disp_enable_ck" onchange="toggleDisplayCfg()"> Enable Display</label>
      </div>
      <div id="display-config" class="fg">
        <div class="f">
  ...
  <div id="pane-out" class="pane">
    ...
    <div class="ib ib-blue" style="font-size:0.72rem">No hard channel limit — constrained by resource scoring + hardware limits | LED Strip: max 8 (RMT limit) | DMX Serial: first 2 use UART, extra ports use RMT fallback | Reboot required after changes.</div>
    ...
    <div id="strapping-warning" class="ib ib-yellow" style="display:none;font-size:0.72rem;border:2px solid #eab308"><strong>Warning:</strong> GPIO 12 is configured as an active pin. GPIO 12 is a Bootstrapping pin (MTDI). If pulled HIGH during controller startup, the board will fail to boot due to flash voltage selection conflict (VDD_SDIO 1.8V vs 3.3V). Ensure your connected device does not drive this pin HIGH on power-up.</div>

    <div class="section">
      <div class="section-head">
        <h3>Configured Channels</h3>
        <div>
          <span id="out-count" class="pill">0 channels</span>
      ...
      <div id="hw-monitor" style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:12px;font-size:0.75rem">
        <div class="sc" style="padding:6px;border-color:#cbd5e1">
          <small style="font-weight:600;color:#475569">RMT Channels</small>
          <strong id="hw-rmt-val" style="font-size:1.1rem;color:#1e3a8a">0 / 8</strong>
          <div style="height:4px;background:#e2e8f0;border-radius:2px;margin-top:4px;overflow:hidden">
            <div id="hw-rmt-bar" style="height:100%;width:0%;background:#22c55e;transition:width .2s"></div>
          </div>
        </div>
        <div class="sc" style="padding:6px;border-color:#cbd5e1">
          <small style="font-weight:600;color:#475569">UART Ports</small>
          <strong id="hw-uart-val" style="font-size:1.1rem;color:#1e3a8a">0 / 2</strong>
          <div style="height:4px;background:#e2e8f0;border-radius:2px;margin-top:4px;overflow:hidden">
            <div id="hw-uart-bar" style="height:100%;width:0%;background:#22c55e;transition:width .2s"></div>
          </div>
        </div>
        <div class="sc" style="padding:6px;border-color:#cbd5e1">
          <small style="font-weight:600;color:#475569">LEDC Channels</small>
          <strong id="hw-ledc-val" style="font-size:1.1rem;color:#1e3a8a">0 / 16</strong>
          <div style="height:4px;background:#e2e8f0;border-radius:2px;margin-top:4px;overflow:hidden">
            <div id="hw-ledc-bar" style="height:100%;width:0%;background:#22c55e;transition:width .2s"></div>
          </div>
        </div>
        <div class="sc" style="padding:6px;border-color:#cbd5e1">
          <small style="font-weight:600;color:#475569">Resource Score</small>
          <strong id="hw-score-val" style="font-size:1.1rem;color:#1e3a8a">0 / 100</strong>
          <div style="height:4px;background:#e2e8f0;border-radius:2px;margin-top:4px;overflow:hidden">
            <div id="hw-score-bar" style="height:100%;width:0%;background:#22c55e;transition:width .2s"></div>
          </div>
        </div>
      </div>
    ...
    <div class="fg">
      ...
      <div class="f" id="no_mc_grp" style="display:none; flex-basis: 100%;">
        ...
        <div class="fg">
          ...
          <div id="rc_filter_grp" style="display:none;flex-basis:100%"><div class="ib ib-purple" style="font-size:0.78rem;margin:8px 0 2px">RC Low-Pass Filter Reference</div><div style="display:flex;flex-wrap:wrap;gap:8px"><div class="f" style="flex:1"><label for="rc_r">R (&Omega;)</label><input type="number" id="rc_r" value="1000" min="1" max="10000000" oninput="calcRcCutoff()"></div><div class="f" style="flex:1"><label for="rc_c">C (nF)</label><input type="number" id="rc_c" value="10" min="1" max="1000000" oninput="calcRcCutoff()"></div><div class="f" style="flex:1"><label>fc ≈ <span id="rc_fc">15915</span> Hz</label><div class="hint">Set PWM Carrier Freq above to 3-5x fc</div></div></div></div>
...
function outputGpios(o){
  ...
  add(o.pin);
  const is7SegDD = (t===12||t===13) && (parseInt(o.mc_mode||0)===2||parseInt(o.mc_mode||0)===3);
...
function outputConfigLabel(o){
  ...
  if(t===14) return 'DAC';
  if(t===15) return `PWM DAC @ ${o.mc_freq||50000}Hz ${o.mc_resolution||8}-bit`;
...
function migrateOutput(o){
  const t = parseInt(o.type);
  if (t === 4) o.type = 4; // PWM Dimmer → Single Color LED (stays)
  else if (t === 5) o.type = 6; // Motor
  else if (t === 6) o.type = 7; // Stepper
  else if (t === 7) o.type = 8; // Servo
  else if (t === 8) o.type = 17; // Solenoid
  else if (t === 9) o.type = 5; // Analog RGB
  else if (t === 10) o.type = 9; // Buzzer
  else if (t === 11) o.type = 18; // Smoke
  else if (t === 12) o.type = 11; // 7-Seg 2-pin
  else if (t === 13) o.type = 10; // DFPlayer
  else if (t === 15) o.type = 15; // PWM DAC
  return o;
}
...
function channelScore(o){
  ...
  const mode=o.mc_mode||0;
  const pin4=o.pin4!==undefined?o.pin4:255;
  const W={gpio:0.5,ledc:2.5,rmt:3.0,uart:8.0,dac:2.0,pca:0.25,exp:0.125};
  let gpio=0,ledc=0,rmt=0,uart=0,dac=0,pca=0,exp=0;
  // Count used UARTs by DFPlayer channels to determine DMX UART availability
  const dfCount=outputs.filter(x=>typeId(x)===10).length;
  switch(t){
    case 0: if(src===0) gpio=1; break;
    case 1:
      if(src===0){
        gpio=1;
        // DFPlayer has priority on UART; DMX falls back to RMT when both UARTs are taken
        const dmxBefore=outputs.slice(0,outputs.indexOf(o)).filter(x=>typeId(x)===1&&(x.source||0)===0).length;
    ...
    case 6:
      ...
      break;
    case 7:
      if(src===0){gpio=2+(pin4!==255&&(o.mc_homing_mode||0)===0?1:0); rmt=2;}
      else if(src===1) pca=2;
      break;
    case 8: if(src===0){gpio=1;ledc=1;}else if(src===1) pca=1; break;
    case 9: if(src===0){gpio=1;ledc=1;} break;
    case 10: if(src===0) gpio=2; uart=1; break;
    ...
    case 18: if(src===0) gpio=2; else if(src===1) pca=2; else exp=2; break;
  }
  return gpio*W.gpio+ledc*W.ledc+rmt*W.rmt+uart*W.uart+dac*W.dac+pca*W.pca+exp*W.exp;
}
function totalScore(arr){
  return arr.reduce((s,o)=>s+channelScore(o),0);
}
function channelComputeScore(o){
  const t=typeId(o);
  switch(t){
    case 3: return (o.led_count||0)*0.005;
    case 6: return 0.5;
    case 7: return 2.0;
    case 10: return 0.5;
    case 11: return 0.5;
...
function fpsComputeFactor(fps){
  fps=parseInt(fps)||40;
  return (fps/60)*5;
}
function totalComputeScore(arr,fps){
  let t=fpsComputeFactor(fps);
  arr.forEach(o=>t+=channelComputeScore(o));
  return t;
}
function resourceScoreLimit(){
  const MAX_GPIO=8, MAX_LEDC=16, MAX_RMT=8, MAX_UART=2, MAX_DAC=0;
  const W={gpio:0.5,ledc:2.5,rmt:3.0,uart:8.0,dac:2.0,pca:0.25,exp:0.125};
  return MAX_GPIO*W.gpio+MAX_LEDC*W.ledc+MAX_RMT*W.rmt+MAX_UART*W.uart+MAX_DAC*W.dac;
}
function totalScoreLimit(){
  return resourceScoreLimit()+25;
}
function updateScoreBar(){
  ...
  const rs=totalScore(outputs);
  const cs=totalComputeScore(outputs,fps);
  const s=rs+cs;
  const limit=totalScoreLimit();
  const pct=Math.min(s/limit*100,100);
  bar.style.width=pct+'%';
  bar.style.background=s>limit?'#ef4444':s>limit*0.8?'#eab308':'#22c55e';
  txt.textContent='R'+rs.toFixed(0)+'+C'+cs.toFixed(0)+'='+s.toFixed(0)+' / '+limit;

  // Calculate hardware resource metrics
  let ledCount = 0;
  let dmxCount = 0;
  let dfPlayerCount = 0;
  let ledcUsed = 0;
  ...
  outputs.forEach(o => {
    ...
    // LEDC PWM usage on local ESP32 GPIO (source === 0)
    if (parseInt(o.source || 0) === 0) {
      if (t === 4) { // Single Color LED
        ledcUsed++;
      } else if (t === 6) { // Motor
        ledcUsed += (parseInt(o.mc_mode || 0) === 0 ? 2 : 1);
      } else if (t === 8) { // Servo
        ledcUsed++;
      } else if (t === 5) { // Analog RGB/RGBW
        ledcUsed += ((o.color_order || 0) >= 4 ? 4 : 3);
      } else if (t === 9) { // Buzzer
        ledcUsed++;
      } else if (t === 16) { // Function Generator
        ledcUsed++;
      } else if (t === 15) { // PWM DAC
        ledcUsed++;
      } else if (t === 12) { // 7-Seg DD 7-Pin PWM
        ledcUsed += 7;
      } else if (t === 13) { // 7-Seg DD 8-Pin PWM
        ledcUsed += 8;
      }
    }
  });
  ...
  const uartUsed = dfPlayerCount + dmxUartUse;
  const rmtUsed = ledCount + dmxRmtUse;

  const rmtLimit = 8;
  const uartLimit = 2;
  const ledcLimit = 16;

  const rmtOver = rmtUsed > rmtLimit;
  const uartOver = uartUsed > uartLimit;
  const ledcOver = ledcUsed > ledcLimit;
  const scoreOver = s > limit;

  const hasError = rmtOver || uartOver || ledcOver || scoreOver;

  // Helper to update each card dynamically
  const updateHwCard = (valId, barId, used, maxLimit) => {
    const valEl = document.getElementById(valId);
    const barEl = document.getElementById(barId);
    if (!valEl || !barEl) return;
    valEl.textContent = `${used} / ${maxLimit}`;
    const cardPct = Math.min(used / maxLimit * 100, 100);
    barEl.style.width = cardPct + '%';
    if (used > maxLimit) {
  ...
  updateHwCard('hw-rmt-val', 'hw-rmt-bar', rmtUsed, rmtLimit);
  updateHwCard('hw-uart-val', 'hw-uart-bar', uartUsed, uartLimit);
  updateHwCard('hw-ledc-val', 'hw-ledc-bar', ledcUsed, ledcLimit);
  updateHwCard('hw-score-val', 'hw-score-bar', s, limit);

  const saveBtn = document.getElementById('out-save-btn');
  if (saveBtn) saveBtn.disabled = hasError;

  if (hasError) {
    let errMsg = 'Limit Exceeded';
    if (scoreOver) errMsg = 'Combined Score (Resource+Compute) Exceeded';
    else if (rmtOver) errMsg = 'RMT Limit Exceeded';
    else if (uartOver) errMsg = 'UART Limit Exceeded';
    else if (ledcOver) errMsg = 'LEDC Limit Exceeded';
...
function renderOutputs(){
  ...
  outputs.forEach((o,i)=>{
    ...
    tr.innerHTML=`<td>${i+1}</td><td>${deviceLabel(o)}</td><td>${gpioLabel(o)}</td><td>${outputAddressLabel(o)}</td>
      <td>${outputChannelCount(o)}</td><td><span class="pill ${scClass}" style="font-size:0.65rem">${sc.toFixed(0)}</span></td>
...
function setModeOptions(kind, type){
  ...
  const opts=kind==='7seg'
    ? type===11?[[0,'TM1637 Numeric (2 Ch)'],[1,'TM1637 ASCII Text (4 Ch)']]
      :type===12?[[2,'Direct 7-pin (A-G)']]
      :type===13?[[3,'Direct 8-pin (A-G+DP)']]
      :[[0,'TM1637 Numeric (2 Ch)'],[1,'TM1637 ASCII Text (4 Ch)'],
       [2,'Direct 7-pin (A-G)'],[3,'Direct 8-pin (A-G+DP)']]
...
function editOutput(idx){
  ...
  setChk('no_pin_invert',o.mc_step_invert||o.pin_invert||false);
  setChk('no_pin2_invert',o.mc_dir_invert||o.pin2_invert||false);
  setChk('no_pin3_invert',o.mc_enable_active_high||o.pin3_invert||false);
...
function addOrUpdateOutput(){
  ...
  // No hard channel limit — constrained by resource scoring + hardware limits
  ...
  // Expander duplicate channel validation
  {
    ...
    for (let i = 0; i < newOutputs.length; i++) {
      ...
      const channels = [o.pca_channel];
      if (o.type === 6 || o.type === 7 || o.type === 5 || o.type === 18) { if(o.pca_channel2!==255) channels.push(o.pca_channel2); }
      if (o.type === 7 || o.type === 5 || (o.type === 6 && parseInt(o.mc_mode||0) === 2)) { if(o.pca_channel3!==255) channels.push(o.pca_channel3); }
      if (o.type === 5 && parseInt(o.color_order||0)>=4) { if(o.pca_channel4!==255) channels.push(o.pca_channel4); }
    ...
    for (let i = 0; i < newOutputs.length; i++) {
      ...
      let freq = 1000;
      if (o.type === 8) freq = 50;
      else if (o.type === 4 || o.type === 6 || o.type === 5 || o.type === 18) freq = parseInt(o.mc_freq || 1000);
      ...
      if(o.type===4||o.type===6||o.type===5||o.type===18){
  ...
  newOutputs.forEach(o => {
    ...
    else if(o.type===15){ if(o.source===0) ledcUsed++; }
  });
  ...
  // Resource + Compute scoring check
  const fps=parseInt(document.getElementById('output_fps')?.value)||40;
  const rs=totalScore(newOutputs);
  const cs=totalComputeScore(newOutputs,fps);
  const sc=rs+cs;
  const lim=totalScoreLimit();
  if(sc>lim){
    alert('Combined score R'+rs.toFixed(0)+'+C'+cs.toFixed(0)+'='+sc.toFixed(0)+' exceeds limit of '+lim+' at '+fps+' FPS. Reduce channels or FPS.');
    return;
  }
