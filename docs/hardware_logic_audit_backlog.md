...
13. **DMX hardware scoring overcounts UART (no RMT fallback awareness)**
    - Files: `include/scoring.h`, `web/index.html`
    - Status: ✅ Completed (`fix(scoring): DMX UART/RMT fallback-aware counting`)
    - Issue: `totalHardware()` and `totalHardwareFromJson()` always count every DMX output as `uart=1`. With 2 DFPlayers + 1 DMX, the hardware budget reads `uart=3 > 2` → blocks config save at scoring, even though runtime `setupChannels()` correctly falls back to RMT when UARTs are exhausted.
    - Fix direction: mirror `totalRam()` fallback logic: count DMX and DFPlayer, allocate remaining UARTs to DMX, put excess DMX on RMT (`t.rmt += dmxRmtUse`). Both C++ aggregate functions and JS `totalHardware()` updated.

14. **Expander/Non-GPIO `pca_addr` lost on edit**
    - Files: `web/index.html`
...
15. **DAC type 14 renders a non-standard two-row layout**
    - Files: `web/index.html`
    - Status: ✅ Completed (`fix(ui): expander addr restore on edit + DAC single-row layout`)
...
19. **Output test path bypasses Core 1 ownership**
    - Files: `src/main.cpp`
...
20. **Recovery mode is Ethernet-only**
...
21. **Web UI reserved-pin validation parity gaps**
...
22. **Web UI settings fields need stronger validation and state feedback**
    - Files: `web/index.html`
...
23. **Web UI diagnostics/header labels are inconsistent**
    - Files: `web/index.html`
...
24. **Web UI usability polish backlog**
    - Files: `web/index.html`
