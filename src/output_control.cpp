...
uint16_t OutputControl::getPcaSharedFrequency(uint8_t address) const {
...
uint16_t OutputControl::sharedPcaFrequency(uint8_t address) const {
    return getPcaSharedFrequency(address);
}

uint16_t OutputControl::getUniverseCount(const OutputChannel& ch) const {
...
bool OutputControl::mapDmxDataToChannels(uint16_t universe, const uint8_t* data, uint16_t length, bool updateActiveBuffer, uint16_t dataOffset) {
...
void OutputControl::swapBuffers() {
...
void OutputControl::begin() {
    loadChannels();
    setupChannels();
    Serial.println("Output Control initialized");
}

void OutputControl::loadChannels() {
    clearChannels();
...
bool OutputControl::saveChannels() {
...
void OutputControl::clearChannels() {
    for (auto& ch : channels) {
...
void OutputControl::updateLeds() {
    ledStripUpdate();
    extern void updateMotionControl();
    updateMotionControl();
}

void OutputControl::setupChannels() {
    uint8_t rmtIdx = 0;
    uint8_t dmxIdx = 0;
...
std::vector<OutputChannel>& OutputControl::getChannels() {
    return channels;
}
