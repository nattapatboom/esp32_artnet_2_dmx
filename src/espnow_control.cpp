...
bool EspNowControl::parseMacAddress(const char* macStr, uint8_t* macBytes) {
    int values[6];
...
uint16_t EspNowControl::clampDmxAddress(int value, uint16_t fallback) {
    if (value < 1 || value > 512) return fallback;
    return (uint16_t)value;
}

bool EspNowControl::peerAllowsUniverse(const PeerRoute& peer, uint16_t universe, uint16_t& firstOffset, uint16_t& lastOffsetExclusive) {
...
void EspNowControl::sendDmx(uint16_t universe, const uint8_t* dmxData, uint16_t length) {
...
void EspNowControl::onDataRecv(const uint8_t* mac, const uint8_t* incomingData, int len) {
    size_t minHeaderLen = 4 + 2 + 2 + 2 + 2;
