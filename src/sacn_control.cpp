...
uint16_t SACNControl::readUint16BE(const uint8_t* buf, int offset) {
    return ((uint16_t)buf[offset] << 8) | buf[offset + 1];
}

uint32_t SACNControl::readUint32BE(const uint8_t* buf, int offset) {
...
bool SACNControl::validatePduFlagsLength(const uint8_t* buf, int len, int offset) {
...
bool SACNControl::validatePacket(const uint8_t* buf, int len) {
...
bool SACNControl::shouldAcceptSource(const uint8_t* cid, uint8_t priority) {
    int freeSlot = -1;
    int existingSlot = -1;
    int lowestPriSlot = -1;
    int acceptedSlot = -1;
...
void SACNControl::expireSources() {
    uint32_t now = millis();
    for (int i = 0; i < SACN_MAX_SOURCES; i++) {
...
void SACNControl::leaveAllMulticast() {
...
void SACNControl::joinMulticast(uint16_t universe) {
...
void SACNControl::reloadMulticastGroups() {
    leaveAllMulticast();
...
void SACNControl::begin(bool useMulticast) {
...
uint16_t SACNControl::getActiveUniverses() const {
    uint16_t count = 0;
    for (int i = 0; i < SACN_MAX_SOURCES; i++) {
