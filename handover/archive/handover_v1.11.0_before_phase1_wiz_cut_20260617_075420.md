...
Items to consider cutting from firmware:
- WiZ Smart Bulb support.
  - Current state: still present as Output type 4 in v1.10 firmware.
  - Reason to cut: saves flash/RAM and keeps controller focused on wired/field outputs.
  - Risk: if WiZ is still useful in current installs, remove later instead of first.
  - Decision needed before coding: cut WiZ now, or keep until PCA9685 is stable.
...
- Optional code cleanup only for confirmed cuts.
...
1. Decide WiZ: keep for now or cut now.
...
4. Before coding, decide whether WiZ removal is truly desired or whether it should remain until PCA/expander support is stable.
