...
- `include/i2c_devices/` — one file per I2C device type:
  ...
  - `pwm_expander.h` — PwmExpanderDriver/PwmExpanderManager for I2C PWM expanders (PCA9685, PCA9635, SN3218, AW9523)
...
- I2C device families (PCA9685, I2C DACs, digital GPIO expanders, displays where practical) must share a common I2C base layer for bus locking, address validation metadata, device lookup/registration, and route handling. Keep per-chip code limited to protocol-specific register/data transactions so adding a new IC does not require duplicating manager, mutex, validation, scoring, or Web UI plumbing.
...
- Shared output behavior must be expressed as common metadata/config flags in the header source of truth, not as scattered type checks. For example, LED Strip and DMX start mapping at the first channel of `start_universe` instead of `start_address` by setting the `OutputModeDef::startAtFirstUniverse` capability in `OUTPUT_MODES[]`; consume that flag in validation, scoring, Web UI generation, and runtime mapping. Do not add repeated checks like `type == TYPE_LED_STRIP || type == TYPE_DMX` across code paths when a metadata capability can describe the behavior.
