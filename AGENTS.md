...
- Shared output behavior must be expressed as common metadata/config flags in the header source of truth, not as scattered type checks. For example, if LED Strip and DMX should start mapping at the first channel of `start_universe` instead of `start_address`, add a boolean-style capability such as `start_at_first_universe` to `OutputModeDef`/`OUTPUT_MODES[]` and consume that flag in validation, scoring, Web UI generation, and runtime mapping. Do not add repeated checks like `type == TYPE_LED_STRIP || type == TYPE_DMX` across code paths when a metadata capability can describe the behavior.
